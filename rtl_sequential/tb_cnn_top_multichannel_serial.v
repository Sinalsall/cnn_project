...
module tb_cnn_top_multichannel_serial;
    ...
    reg signed [DATA_WIDTH-1:0] mem_resp_data;
    reg [15:0] param_rd_addr;
    wire [15:0] param_rd_data;
    reg param_wr_en;
    ...
    always @(*) begin
        if (mem_req_kind == MEM_KIND_BIAS) begin
            case (mem_req_layer)
                L_CONV1: param_rd_addr = BASE_CONV1_B + mem_req_addr;
                L_CONV2: param_rd_addr = BASE_CONV2_B + mem_req_addr;
                L_CONV3: param_rd_addr = BASE_CONV3_B + mem_req_addr;
                L_CONV4: param_rd_addr = BASE_CONV4_B + mem_req_addr;
                L_FC:    param_rd_addr = BASE_FC_B    + mem_req_addr;
                default: param_rd_addr = 16'd0;
            endcase
        end else begin
            case (mem_req_layer)
                L_CONV1: param_rd_addr = BASE_CONV1_W + mem_req_addr;
                L_CONV2: param_rd_addr = BASE_CONV2_W + mem_req_addr;
                L_CONV3: param_rd_addr = BASE_CONV3_W + mem_req_addr;
                L_CONV4: param_rd_addr = BASE_CONV4_W + mem_req_addr;
                L_FC:    param_rd_addr = BASE_FC_W    + mem_req_addr;
                default: param_rd_addr = 16'd0;
            endcase
        end

        mem_resp_data = param_rd_data;
    end
    ...
    task preload_param_sram;
        begin
            $display("[TB] Preloading CNN parameters into 1024x16 SRAM banks...");
            ...
            $display("[TB] Parameter SRAM preload done. Last used word address = %0d", BASE_FC_B + 9);
        end
    endtask
    ...
    initial begin
        if ($test$plusargs("DUMP_VCD")) begin
            $dumpfile("rtl_sequential/tb_cnn_top_multichannel_serial.vcd");
            $dumpvars(0, tb_cnn_top_multichannel_serial);
        end

        $display("=== CNN TOP MULTICHANNEL SERIAL TB START ===");
        ...
        $display("=== CNN TOP MULTICHANNEL SERIAL TB DONE ===");
        #20 $finish;
    end
endmodule
