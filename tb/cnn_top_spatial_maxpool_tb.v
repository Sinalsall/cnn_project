...
module cnn_top_spatial_maxpool_tb;
    ...
    initial begin
        $dumpfile("tb/cnn_top_spatial_maxpool_tb.vcd");
        $dumpvars(0, cnn_top_spatial_maxpool_tb);

        $display("=== CNN TOP SPATIAL MAXPOOL TB START ===");
        ...
        // Input image row-major:
	// pixel[0] = 1, pixel[1] = 2, ..., pixel[783] = 784.
	// Setelah dua kali MaxPool 2x2, output akhir 7x7 adalah
	// maksimum setiap blok 4x4.
        for (i = 0; i < 28*28; i = i + 1) begin
            set_image_pixel(i, i + 1);
        end
        ...
        // 0 0 0
        //
        // Aktifkan hanya channel 0 sepanjang pipeline.
        ...
        // FC class 0 menjumlahkan flatten input index 0..48.
        // Asumsi flatten channel-major: channel 0, 7x7 berada di index 0..48.
        ...
        $display("[TB] Starting identity-path inference...");
        pulse_start();
        ...
        if (error_count == 0) begin
            $display("[TB][PASS] top-level spatial MaxPool/flatten test passed.");
        end else begin
            $display("[TB][FAIL] Found %0d mismatches.", error_count);
            $finish;
        end

        $display("=== CNN TOP IDENTITY-PATH TB END ===");
        $finish;
    end
